...
naglowki_numeryczne <- names(dane)[kolumny_numeryczne]
naglowki_bez_populacji <- naglowki[!grepl("^population", naglowki)]
naglowki <- names(dane)

naglowki
kolumny_numeryczne

dane_numeryczne <- dane %>% select(naglowki_numeryczne)
```


```{r}
drzewo <- ctree(municipality_code~., data=dane_numeryczne, 
                controls = ctree_control(maxdepth=3))
plot(drzewo)
```

```{r}
#drzewo <- rpart(municipality_name~.,data=dane, maxdepth=3)
#rpart.plot(drzewo)
```
