...
```{r}
daneSzczep <- dane %>% select(c(1,2,4)) %>% rename(mncplty_c=1, mncplty_n=2, ProcZaszczepionych=3) %>% mutate(ProcZaszczepionych=as.numeric(ProcZaszczepionych)) %>% 
mutate(mncplty_c=as.character(mncplty_c))
head(daneSzczep)
```
